...
function draw_help_window()
    if MACRO.showHelp then
        if ImGui.Begin(ctx, 'Ambient 1 - Envelopes for Airports Help', true) then
            ...
            ImGui.Text(ctx, '4. Use XY pad for organic character control')
            ImGui.Text(ctx, '5. Auto-apply generates modulation instantly')
            ...
            ImGui.SameLine(ctx)
            ImGui.Text(ctx, ' Self-similar recursive noise patterns')
            ImGui.TextColored(ctx, 0xFF6666FF, '• Sine Wave Interference:')
            ImGui.SameLine(ctx)
            ImGui.Text(ctx, ' Multiple sine waves with beating patterns')
            ImGui.TextColored(ctx, 0xFF6666FF, '• Generative Walk:')
            ImGui.SameLine(ctx)
            ImGui.Text(ctx, ' Rule-based segment movement with momentum')
            ImGui.TextColored(ctx, 0xFF6666FF, '• L-Systems:')
            ImGui.SameLine(ctx)
            ImGui.Text(ctx, ' Recursive string rewriting with organic growth')
